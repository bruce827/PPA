...
function scoreCandidate(name, url) {
  ...
  if (/中国|集团|有限责任公司|有限公司|公司/.test(name)) score += 2;
  if (/电子采购平台|电子商务平台|采购平台|集中采购平台|采购管理平台/.test(name)) score += 2;
  if (/电子招采平台$/.test(name)) score -= 3;
  if (/^哈电/.test(name)) score -= 1;
...
async function main() {
  ...
  const sourceRows = allRows.filter((row) => row.review_bucket === 'enterprise_procurement_platform');

  const pending = [];
  const canonicalBySoftKey = new Map();

  for (const row of sourceRows) {
    if (/^电子招采平台$/.test(row.site_name)) {
      pending.push({ name: row.site_name, url: row.url, reason: 'generic-name' });
      continue;
    }
