...
在PPA项目评估系统中新增"模型配置"模块，为系统提供AI能力支撑。用户可以配置多个AI模型连接，管理提示词模板，为风险分析、模块梳理、工作量评估、Web3D Step4 图片识别分析等能力提供基础设施。

### 1.2 业务价值

- **灵活性**: 支持多种AI服务商（OpenAI、阿里云、百度、Google Gemini、MiniMax等），快速切换测试不同模型效果
- **多模态扩展**: 支持单独维护“当前视觉模型”，服务 Web3D Step4 图像识别与工作量映射
...
用户可以配置多个AI模型的连接信息（API Key、Host等），并分别选择“当前通用模型”和“当前视觉模型”。系统会将用户的选择应用到对应的 AI 功能中。
...
- Timeout（超时时间，默认30秒）
- 是否支持图片识别（`supports_vision`）
- 是否为当前视觉模型（`is_current_vision`）
...
- **当前通用模型**：供通用文本 AI 功能使用
- **当前视觉模型**：供 Web3D Step4 图片识别与工作量映射使用

**业务规则**:
- 用户可配置多个模型连接
- 同一时间只能有**一个**模型为“当前通用模型”
- 同一时间只能有**一个**模型为“当前视觉模型”
- 点击“设为当前”切换当前通用模型
- 点击“设为当前视觉模型”切换当前视觉模型
- 只有 `supports_vision = true` 的模型才能被设置为当前视觉模型
- 当前使用的模型在列表中标记⭐，当前视觉模型额外显示“视觉”状态
...
> 对开启 `supports_vision` 的模型，测试连接只校验服务可达与模型可用；真实图片识别效果以 `/web3d/new` Step4 的 AI 分析为准。
...
interface AIModelConfig {
  ...
  is_current_vision: boolean; // 是否当前视觉模型
  is_active: boolean;        // 是否启用
  supports_vision: boolean;  // 是否支持图片识别
...
管理AI功能使用的提示词模板。系统预置常用模板（如风险分析、工作量评估、模块梳理），用户也可以创建自定义模板。模板支持变量占位符，运行时动态替换。
...
模板不再使用旧的 `category` CHECK 枚举，改为通过 `module_tag` 字段关联 `prompt_module_tags` 字典表。

推荐标签（6个）：

| value | label | 说明 |
|---|---|---|
| `assessment` | 评估流程 | 评估流程中的 AI 分析（风险分析、工作量评估、模块梳理） |
| `web3d` | 3D 模块 | Web3D 模块的 AI 分析 |
| `tender` | 招标信息 | 招标信息的解析、字段提取等 |
| `bidding_search` | 全网招标检索 | 全网招标信息的搜索与处理 |
| `report` | 报告生成 | 评估报告的生成 |
| `general` | 通用 | 通用模板，无特定模块归属 |

用户也可创建自定义 `module_tag`，推荐标签不可删除，自定义标签可自由管理。
...
`web3d` 标签下的模板仅用于 `/web3d/new` Step4 的 AI 模块梳理与工作量映射，由后端自动追加 Step1-3 上下文、当前工作量模板全集、角色配置和图片元数据。
...
interface PromptTemplate {
  ...
  module_tag: string;          // 模块标签（关联 prompt_module_tags.value）
...
│  ├────────────────────────────────────────────────┤    │
│  │ 项目风险分析  │风险分析   │系统│  5   │启用│[…]│    │
│  │ 项目工作量评估│工作量评估 │系统│  6   │启用│[…]│    │
│  │ Web3D Step4   │Web3D分析  │系统│  3   │启用│[…]│    │
│  │ 我的分析模板  │自定义     │用户│  3   │启用│[…]│    │
│  └────────────────────────────────────────────────┘    │
│                                                          │
└─────────────────────────────────────────────────────────┘
...
|------|------|------|
| 模板名称 | 30% | 模板的名称 |
| 分类 | 20% | 风险分析/工作量评估/模块梳理/Web3D Step4 分析/自定义等 |
| 类型 | 12% | 系统/用户 |
| 变量数 | 12% | 包含的变量数量 |
| 状态 | 10% | 启用/禁用 |
...
#### 模板2: Web3D Step4 全量评估

```yaml
template_name: Web3D Step4 全量评估
category: web3d_step4_analysis
description: 用于 Web3D Step4，结合 Step1-3 上下文、工作量模板、角色配置和图片输出 coverage 与 step4_rows

system_prompt: |
  你是一个资深 Web3D 项目评估专家。
  你必须对输入中的全部 Web3D 工作量模板项逐项评估，并严格逐字复制 category、item_name、role_name。
  你的输出会被程序严格校验，只允许输出合法 JSON。
...
user_prompt_template: |
  ...
  请仅输出以下 JSON：
  {
    ...
    "coverage": [
      {
        "category": "逐字复制 workload_templates 中的原值",
        "item_name": "逐字复制 workload_templates 中的原值",
        "applicability": "required",
        "recommended_base_days": 3,
        "recommended_delivery_factor": 1.2,
        "recommended_role_names": ["逐字复制 roles 中的原值"],
        "reason": "中文原因"
      }
    ],
    "step4_rows": [
...
variables:
  - name: business_goal
    description: 业务目标
    example: 用于展厅大屏演示和领导汇报
    required: false
  - name: focus_points
    description: 重点关注项
    example: 模型展示效果、基础交互、普通PC流畅运行
    required: false
  - name: extra_constraints
    description: 额外约束
    example: 优先保证按时上线，不做过重特效
    required: false
```
...
```
GET    /api/config/prompt-module-tags    # 获取模块标签字典（新增）
GET    /api/config/prompts                # 获取模板列表（支持 module_tag 前缀模糊匹配）
...
CREATE TABLE ai_model_configs (
  ...
  is_current_vision BOOLEAN DEFAULT 0, -- 是否当前视觉模型
  is_active BOOLEAN DEFAULT 1,         -- 是否启用
  supports_vision BOOLEAN DEFAULT 0,   -- 是否支持图片识别
...
-- 确保只有一个当前视觉模型
CREATE UNIQUE INDEX idx_current_vision_model
...
CREATE TABLE prompt_templates (
  ...
  module_tag TEXT NOT NULL,            -- 模块标签（关联 prompt_module_tags.value）
...
-- 创建索引
CREATE INDEX idx_prompt_module_tag
  ON prompt_templates(module_tag);
...
### 5.3 表3: prompt_module_tags（新增）

```sql
CREATE TABLE prompt_module_tags (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  value TEXT NOT NULL UNIQUE,          -- 标签值，如 'assessment'、'web3d'
  label TEXT NOT NULL,                 -- 中文展示名，如 '评估流程'、'3D 模块'
  description TEXT,                    -- 标签描述
  is_recommended INTEGER NOT NULL DEFAULT 0,  -- 是否推荐（推荐标签不可删除）
  sort_order INTEGER NOT NULL DEFAULT 0,      -- 排序顺序
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
);

CREATE UNIQUE INDEX idx_module_tags_value
  ON prompt_module_tags(value);
```
...
迁移文件 `013` 将旧的 `category` CHECK-enum 数据迁移到 `module_tag`，`014` 创建新的 `prompt_module_tags` 字典表并预置 6 个推荐标签。

```javascript
// 013_prompt_template_category_to_moduletag.js
// 014_prompt_module_tags_dict.js
```
...
- ✅ 设置当前视觉模型
- ✅ 连接测试功能
- ✅ 视觉能力位（supports_vision / is_current_vision）
- ❌ ~~API密钥加密存储~~（简化：明文存储）

**提示词模板管理**:
- ✅ 基础CRUD功能
- ✅ 支持 Web3D Step4 专用分类（web3d_step4_analysis）
- ✅ 变量定义和管理
...
- **运行时**: Node.js + Express
- **数据库**: SQLite3
- **图片能力**: Gemini / MiniMax Vision Provider
...
- `aiModelService.js` - 模型配置与连接测试服务
- `geminiVisionProvider.js / minimaxVisionProvider.js` - 视觉模型 Provider
...
server/
  controllers/
    ...
    promptTemplateController.js  # 模板控制器
  services/
    aiModelService.js            # 模型与测试服务
  providers/ai/
    geminiVisionProvider.js      # Gemini视觉模型
    minimaxVisionProvider.js     # MiniMax视觉模型
  migrations/
    001_create_ai_model_configs.js
    002_create_prompt_templates.js
    003_create_ai_prompts_table.js
    010_add_vision_flags_to_ai_model_configs.js
    013_prompt_template_category_to_moduletag.js
    014_prompt_module_tags_dict.js
```
...
- ✅ 设置当前视觉模型（唯一性）
- ✅ 连接测试（成功/失败场景）
- ✅ supports_vision / is_current_vision 字段读写
...
**Q: 支持哪些AI服务商？**
A: 目前支持 OpenAI 系兼容模型，并已覆盖 Google Gemini、MiniMax 等可用于视觉分析的模型配置场景。

**Q: API Key会泄露吗？**
A: 个人使用场景下，API Key 存储在本地数据库中。建议通过文件系统权限保护数据库文件，或者将敏感配置放在 `.env` 文件中（已在 `.gitignore` 排除，不会提交到 Git）。

**Q: 可以同时使用多个模型吗？**
A: 可以配置多个模型，但同一时间只能有一个“当前通用模型”和一个“当前视觉模型”。视觉模型主要用于 `/web3d/new` Step4 的图片识别分析。
...
- ✅ 模型应用管理的所有P0功能已实现
- ✅ 提示词模板管理的所有P0功能已实现
- ✅ 通用模板分类与 `web3d_step4_analysis` 分类已支持
- ✅ API接口完整且符合RESTful规范
- ✅ 数据库表结构完整
...
- v1.2 (2026-04-19): 提示词模板分类从 `category` CHECK-enum 重构为 `module_tag` + `prompt_module_tags` 字典表，新增 `GET /api/config/prompt-module-tags` 端点，移除旧分类枚举
