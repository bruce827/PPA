...
- **版本号**: V1.1.0
- **创建日期**: 2025-12-01
- **状态**: 持续优化（核心流程已落地，按反馈迭代 UI/导出/配置体验）
...
- **表格列**：Step4 展示类别/工作项/角色（多选均价）/人天/交付系数/小计/操作。Step5 工作量明细仅保留“类别（中文）/工作项/工作量（人天，居中）/小计(万元，右对齐)”。
...
- 风险评分汇总及风险等级
- 工作量估算明细（按 A/B/C/D 分类，显示角色均价/人天/交付系数小计）
...
> 前端已实现：Step1-3 风险评分 + Step4 工作量（角色多选均价、交付系数）→ Step5 报价显示“基础成本×风险系数”；导出支持 Web3D 格式。
