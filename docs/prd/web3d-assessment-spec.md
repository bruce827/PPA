...
- **创建日期**: 2025-12-01
- **最后更新**: 2026-04-07
- **状态**: 持续优化（核心流程、Step4 AI 辅助映射与导出链路已落地，按反馈迭代提示词与交互细节）
...
3. 识别数据资产风险（最大的坑点）
4. 输出准确的工作量估算和成本报价
5. 在 Step4 提供 AI 辅助模块梳理与工作量映射能力，允许“先映射可命中项，再人工补充未命中项”
...
> 日单价/角色单价来源：统一从“参数配置/角色与单价管理”读取，不在 Step1 显示输入。工作量阶段（Step4）每个工作项选定角色与数量，由角色单价驱动成本；Step5 统一应用风险系数进行加权。
...
#### 3.2.4 表单字段与校验（遵循 8 页面原型）

- **项目基础信息**：项目名称（必填，首行）；同一行放置“模型来源与版权”（必填，无默认值，选项含“设计院可以给出BIM数据，但是需要贴图”），下方单行输入“模型来源备注”记录格式/贴图/版权说明；项目描述放在末尾。
- **可视距离范围**：使用 AntD `Slider range` 组件，范围 0.5~8 米，默认值 0.5~1 米，显示刻度与实时数值。下方提示语精简为“一行”：大屏 3-6 米；触摸墙/汇报 0.8-1.5 米；坐席 0.5-0.7 米。
- **目标终端与显卡**：必填多选（PC/大屏/iPad/手机/大屏一体机等），默认“普通 PC”；提示移动端/iPad 需大幅减面并关闭特效。
- **场景背景必答问题**（与项目名称同区域呈现）：
  1) 用户最远/最近观看距离是多少？  
  2) 目标终端与显卡类型？（高性能显卡 vs iPad 等移动端）  
  3) 模型来源与版权归属？是否需重建/精修？
  > 目的：决定 LOD/贴图精度、特效开关、以及是否预留从零建模费用。
- **校验**：点击“下一步”或“重新计算/保存”时，至少校验项目名称、目标终端、模型来源必填。
...
#### 3.5.0 Step4 顶部 AI 模块梳理 / 工作量映射

Step4 顶部新增独立的 **AI 模块梳理** 卡片，用于把结构化上下文与图片一起发送给视觉模型，辅助生成工作量评估结果。

**能力范围**:

- 选择 `web3d_step4_analysis` 分类的提示词模板
- 填写模板变量
- 上传最多 `3` 张图片
- 自动拼接 Step1-3 的结构化表单上下文
- 自动附带当前 Web3D 工作量模板全集
- 自动附带当前角色配置

**输入上下文**:

- Step1：项目名称、描述、模型来源、模型来源备注、可视距离、目标终端
- Step2：数据资产相关风险项及已选答案
- Step3：开发需求相关风险项及已选答案
- Web3D 工作量模板全集
- 当前角色配置
- 图片元数据与图片内容
...
- `coverage`：要求对全部工作量模板项逐项给出适用性、人天、交付系数、角色和原因
- `step4_rows`：真正可用于覆盖 Step4 表格的工作量行
- `unmapped_items`：AI 输出中未能精确映射到现有模板的项
- `missing_template_items`：本次未成功映射的模板项，供人工补齐

**应用策略**:

- 不再要求“全量严格命中才允许覆盖”
- 只要存在可映射的 `step4_rows`，就允许覆盖 Step4
- 未映射项不阻断流程，前端明确提示后由用户手工补齐

**依赖约束**:

- 必须存在“当前视觉模型”
- 当前视觉模型需开启 `supports_vision = 1`
- 所有调用必须写入 AI Log，step 固定为 `web3d_step4`
...
- 日单价取值：从“参数配置/角色与单价管理”读取，每个工作项需选择角色，按角色单价 × 工作项人天汇总后再乘风险系数 D。
- 不在 Step1 收集日单价；Step4 支持为每个工作项指定角色/数量，默认读取当前角色配置。
- 工作项选择/角色单价：
  - 工作项列表来源于 Web3D 工作量模板配置，可维护
  - 角色可多选，均价为所选角色单价的平均值（不可手改）
  - 交付系数参与小计：小计(万元)=人天×交付系数×均价（元→万元）
...
> 成本计算时机：Step4 仅展示未加权的工作量；Step5 才套用风险系数 D，叠加角色单价得到最终报价。

#### 3.5.1 表单与交互约束
- **类别/工作项**：类别下拉显示中文（数据处理/核心开发/业务逻辑/性能与兼容性），工作项单选，来源于 Web3D 工作量模板，可维护。
- **AI 卡片位置**：固定在 Step4 工作量表上方，包含模板选择、变量输入、图片上传、上下文预览、coverage 预览与“覆盖到 Step4”按钮。
- **角色与单价**：角色多选取均价（只读，取“角色与单价管理”配置），不可手动编辑；新增行默认选首个角色。
- **人天与交付系数**：人天表示基准工时，交付系数为倍数，小计=人天×交付系数×均价（元→万元，右对齐显示）。
- **原因说明**：Step4 表增加 `原因说明` 列。该字段由 AI 初始化，但允许用户直接编辑；保存后的值视为最终说明，不再区分“AI 原因”和“人工备注”。
- **原因清理规则**：若用户手动修改了类别或工作项，则该行 `原因说明` 自动清空，避免旧说明挂在新工作项上。
- **表格列**：Step4 展示类别/工作项/原因说明/角色（多选均价）/人天/交付系数/小计/操作。Step5 工作量明细展示“类别（中文）/工作项/原因说明/工作量（人天，居中）/小计(万元，右对齐)”。
- **操作入口**：所有重新计算/保存按钮收敛到 Step5 操作区，外层重复按钮已移除。

### 3.6 Step 5: 生成总览与报价

复用现有 Overview 组件的框架，展示：
- 风险评分汇总及风险等级
- 工作量估算明细（按 A/B/C/D 分类，显示角色均价/人天/交付系数小计，并展示 `原因说明`）
- 成本计算：基础成本=角色均价×人天×交付系数汇总；风险系数来自 Step1-3（含混合方案附加），最终 clamp 到 **1.0~3.0**，总成本=基础成本×风险系数
- 导出/重新计算/保存按钮集中在 Step5 右侧操作卡片；支持导出 Excel/PDF

> 前端已实现：Step1-3 风险评分 + Step4 工作量（角色多选均价、交付系数、原因说明）→ Step5 报价显示“基础成本×风险系数”；导出支持 Web3D 格式，并在“工作量明细”中保留 `原因说明`。
...
CREATE TABLE IF NOT EXISTS web3d_workload_templates (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  category TEXT NOT NULL, -- 'data_processing', 'core_dev', 'business_logic', 'performance'
...
### 6.4 Assessment JSON 扩展

Web3D 项目仍然沿用 `projects.assessment_details_json` 存储评估快照，不新增关系型表字段。

Step4 相关扩展约束如下：

- `assessment.workload_items[*].reason`：保存最终的“原因说明”
- `reason` 为可选字段，兼容历史项目
- AI 原始 `coverage / step4_rows / unmapped_items` 不写入项目主表，保留在 AI Log 中用于排查
...
### 7.3 Web3D Step4 AI API

```
GET  /api/web3d/ai/step4-prompts   # 获取 Web3D Step4 AI 提示词模板
POST /api/web3d/ai/step4-analyze   # Step4 AI 分析（multipart/form-data）
```
...
│  Step 4: 工作量估算                                                                              │
├──────────────────────────────────────────────────────────────────────────────────────────────────────┤
│  [AI 模块梳理卡片] 模板选择 / 图片上传(最多3张) / coverage预览 / 覆盖到Step4                      │
│  未映射项会单独提示，已命中项可先覆盖，剩余由人工补齐                                                │
│                                                                                                    │
│  A. 数据处理工时                                                                              │
│  ┌────────────────────────────────────────────────────────────────────────────────────────────┐  │
...
│  A. 数据处理明细                                                                              │
│  ┌────────────────────────────────────────────────────────────────────────────────────────────┐  │
│  │ 工作项              | 原因说明 | 单位 | 数量 | 单位工时 | 工作量小计              │  │
│  ├────────────────────────────────────────────────────────────────────────────────────────────┤  │
│  │ Revit 清洗轻量化 | 需拆构件并做轻量化 | 栋 | 1 | 3.5 天 | 3.5 天               │  │
│  │ 贴图修复         | 需修复材质表现     | 栋 | 1 | 1.5 天 | 1.5 天               │  │
│  │                                                                        小计： 5.0 天  │  │
│  └────────────────────────────────────────────────────────────────────────────────────────────┘  │
│                                                                                                    │
│  B. 核心开发明细                                                                              │
...
└──────────────────────────────────────────────────────────────────────────────────────────────────────┘
```
> 实现进展：已在参数配置页新增“Web3D 风险配置”Tab，支持风险项/工作量模板 CRUD，选项/权重/基准工时均可维护。
...
### 9.4 交付系数获取
- 从模板/历史项目中学习交付系数推荐值
- 按项目类型/角色自动推荐交付系数，并提示来源（经验/历史项目）

### 9.5 AI 生成 SOW
- 支持基于评估数据与模型配置，自动生成项目范围说明书（SOW）草稿，供导出/下载。

### 9.6 详情页评估数据展示
- 现状：历史项目详情页的“评估数据”块已暂时隐藏，待设计更友好的呈现方式（结构化展示或下载）。
- 后续：设计按步骤/字段分组的可读视图，或支持一键导出评估详情。
