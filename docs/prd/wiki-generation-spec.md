...
### 3.6（V2.1）多知识库项目支持与运作流程

为了支持在一套 PPA 系统中管理、切换和查阅多个独立的项目 Wiki 知识库，系统在 V1.8.2 中进行了多项目化重构。

#### 3.6.1 物理存储结构
- 物理根目录在：`server/wiki/`。
- 其下划分多个子文件夹作为独立的知识库项目目录，如：`server/wiki/反盗油专项/`。
- 每个项目目录下维护一套独立的索引文件 `INDEX.md` 和文档附件。

#### 3.6.2 数据获取与检索流程
1. **项目发现与定位**：接口 `GET /api/wiki` 接收可选的 `project` 参数。
   - 后端扫描 `server/wiki/` 根目录下的所有子目录，生成可用知识库列表 `projects`。
   - 若 Query 未传 `project` 参数，则默认使用字母序的第一个目录作为 `currentProject`。
2. **目录树扫描与去重**：
   - 读取并解析 `currentProject` 目录下的 `INDEX.md` 文件。
   - 物理扫描该项目子目录下的所有 `.md` 文件，找出未在 `INDEX.md` 中注册的物理文档，合并追加到目录树末尾并标记 `inIndex: false`。
   - 生成树节点的 `wiki_key` 时自动附加当前项目名前缀（格式为：`{项目名}/{相对路径}`，如 `反盗油专项/01_储油罐渗漏检测/储油罐渗漏检测.md`），以此在关联多对多数据库时作为唯一标志键。
   - 将生成的项目目录树缓存至后端的 `wikiTreeCacheMap` 中（生存周期 10 秒），防磁盘高频 I/O 阻塞。
3. **接口响应封包**：后端返回数据格式为：
   ```json
   {
     "success": true,
     "data": [ ... ], // 当前项目的目录树节点数组
     "projects": ["反盗油专项", "其它知识库"], // 系统发现的全部知识库项目
     "currentProject": "反盗油专项" // 当前正在读取的知识库项目
   }
   ```

#### 3.6.3 Obsidian 双链与图片转换流程
当请求特定的文档正文（`GET /api/wiki/content?path={wiki_key}`）时：
1. **路径防越界校验**：后端利用 `path.resolve` 和 `path.relative` 进行强卡点校验，非 `server/wiki/` 内的文件一律拦截并返回 403，防范 Path Traversal 路径穿越攻击。
2. **多项目路径重组**：转换器根据文件的相对父目录（包含项目名前缀，如 `反盗油专项/01_储油罐渗漏检测`），自动为文档内转换后的 Markdown 节点补充项目名：
   - Obsidian 直嵌图片 `![[img.png]]` 替换为静态资源托管路由：`![img.png](/api/wiki/images/项目名/当前目录/img.png)`。
   - 页面内部双链 `[[Link]]` 替换为无刷新页面跳转路由：`[Link](/form-design/wiki?key=项目名/当前目录/Link)`。
   - 相对图片地址重组，自动补齐项目名前缀。

#### 3.6.4 前端页面多项目联动流程
1. **下拉项目切换**：左侧树上方渲染“知识库项目”的 `Select` 选择器，选项来源于后端接口返回的 `projects`。
2. **URL 路由同步**：用户切换下拉选项时，调用 `setSearchParams({ project: 选中的项目 })`。这会触发 React 的 URL 参数监听钩子，重新请求后端对应的项目目录树。
3. **安全自愈首选**：切换项目后，若 URL 上的激活文档 `key` 不存在于该新项目的目录树中，页面会自动重置路由到新项目的第一篇文档并激活加载，避免发生界面白屏。
