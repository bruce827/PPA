...
历史项目管理模块提供已完成项目评估的查看、管理和操作功能。用户可以浏览所有历史项目，进行搜索、筛选、编辑、删除、查看详情、生成商务报价和导出报告等操作。

> 补充说明：历史项目页中的 `商务报价` 按钮、`商务报价状态` 筛选、`商务报价总计` 展示以及 `导出Excel（商务）` 的完整规则，详见 [business-pricing-spec.md](./business-pricing-spec.md)。
