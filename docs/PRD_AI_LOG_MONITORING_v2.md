...
| 1.2 | 2026-04-07 | 补充 Web3D Step4 AI 日志、图片元数据与视觉模型相关监控说明 | Codex |
...
| Step | AI调用类型,如risk、modules、workload、risk-normalize、model-test、web3d_step4等 |
...
| Step | AI调用类型(risk/modules/workload/risk-normalize/model-test/web3d_step4等) | 是 | 是 | - |
| 请求Hash | 唯一标识(短格式) | 是 | 是 | - |
...
| Step | 多选下拉框 | risk/modules/workload/risk-normalize/model-test/web3d_step4 |
...
- 语法高亮和格式化
- 支持复制
- 对 `web3d_step4` 额外展示 `coverage`、`step4_rows`、`unmapped_items`、`missing_template_items`
...
- ...
- 对 `web3d_step4`，如本次调用上传图片，还可能包含 `images/` 子目录
...
{
  type: "subscribe",
  steps: ["risk", "modules", "workload", "web3d_step4"]  // 订阅的step类型
}
...
- endDate: 结束日期(ISO格式)
- steps: Step类型数组(risk,modules,workload,risk-normalize,model-test,web3d_step4...)
...
- 对 Web3D Step4 调用,日志需同时记录图片元数据、最终 prompt、解析结果以及未映射项统计
...
│  ┌───────────────────────────────────────────────────────┐  │
│  │ 筛选条件                                               │  │
│  │ 时间范围: [2025-01-01]~[2025-01-31]  Step: [风险分析 / Web3D Step4▼]│  │
...
│  │ 2026-04-07... web3d_step4 d5afc6df... Gemini succ 6.8s│ │
│  │ ...                                                   │  │
│  │                                          [刷新]        │  │
│  └───────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
...
│  Step: web3d_step4  Hash: d5afc6df083a...                  │
│                                                             │
│  ┌───────────────────────────────────────────────────────┐  │
...
│  │ • 路由地址: POST /api/web3d/ai/step4-analyze         │  │
...
│  │                                                       │  │
│  │ • 文件路径:                                            │  │
│  │   - server/logs/ai/web3d_step4/2026-04-07/...         │  │
│  │   - server/logs/ai/web3d_step4/2026-04-07/.../images/ │  │
│  │                                                       │  │
│  └───────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
```
...
```json
{
  ...
  "timestamp": "2026-04-07T00:24:10.112Z"
}
```
  ...
  "images": [
    {
      ...
      "size": 582133,
      "sha256": "...",
      "relativePath": "images/scene-1.png"
    }
  ]
}
```
...
- [ ] 可以查看单条日志的完整详情
- [ ] 可以查看请求内容、响应内容和错误分析
- [ ] `web3d_step4` 日志可查看图片元数据、未映射项和 `images/` 子目录信息
- [ ] 统计面板显示正确
