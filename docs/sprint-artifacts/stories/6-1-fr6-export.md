...
4. **元数据与流式输出**：Workbook 属性写入 `creator/created/modified`，Summary Sheet 展示 `snapshot_id/config_version/exported_at/rating_factor`，全流程使用 exceljs 流式写入 HTTP 响应，禁止落地临时文件。（[Source: docs/prd/export-spec.md#fr-6-3-导出元数据与追溯](docs/prd/export-spec.md#fr-6-3-导出元数据与追溯)）
