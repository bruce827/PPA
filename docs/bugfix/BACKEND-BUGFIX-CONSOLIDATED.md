...
| 日期 | 变更内容 | 相关 Sprint |
|------|---------|------------|
| 2026-07-01 | 修复 Supabase PostgreSQL 连接串与 SSL 配置问题 | - |
...
### 7.4 Supabase PostgreSQL 连接串与 SSL 配置问题（2026-07-01）

**故障现象**:  
后端以 `DB_TYPE=postgres` 启动时，先出现 `pg-connection-string` SSL 模式警告，随后 PostgreSQL 连接失败：
...
1. 新增 `server/utils/postgresConfig.js`，在交给 `pg` 前从连接串中移除 `sslmode` / `uselibpqcompat` 查询参数，并由应用代码显式设置 `ssl`。
2. `server/utils/db.js`、`server/scripts/migration/lib.js`、`server/scripts/migration/apply-slow-query-indexes.js` 统一复用 `buildPostgresConnectionConfig()`。
3. 将真实环境中的 `DATABASE_URL` 从 pooler 地址切换为 direct connection，密码沿用原值：
...
**预防措施**:
- Supabase PostgreSQL URL 优先使用 direct connection 格式。
- 如需使用 pooler，必须从 Supabase 控制台重新复制当前项目的 pooler URI，避免项目引用、区域、端口或用户名格式不匹配。
- 不要把 `sslmode=require` 放进 `DATABASE_URL`；统一通过 `PGSSLMODE=require` 或连接配置 helper 控制 SSL。
- 启动报 `Connection terminated unexpectedly` 时，分别探测 pooler 与 direct DB；如果 direct DB 可通而 pooler 不通，优先判断为 pooler 配置问题。
