...
- **IoT 点位对接评估 V1.8**：在标准 `/assessment/new` 工作量估算步骤中新增 `IoT点位对接` Tab，按已有 IoT 平台/网关能力进行手动规模评估，独立输出 IoT 点位对接成本、人天和 Excel 明细。
- **大屏原型 Agent 协作与双向同步 V2.0**：新增对第三方协作 Agent 的 5 个 OpenAPI 安全接口，实现极简指标大纲生成、Canvas 12栅格自适应推荐布局、网格坐标与 Web3D 视角参数双向同步、一键转化为 PPA 项目成本模板等核心功能。
...
4. **专业导出能力**：支持一键生成 Excel 报告，与实时计算结果保持一致，为投标、复审或对外报价提供权威材料。

---

## PRD 索引（细分 PRD）

- [Wiki 生成模块规格 V1.9](./prd/wiki-generation-spec.md)
...
- [IoT 点位对接评估 V1.8 规格](./prd/iot-point-integration-assessment-spec.md)
- [表单设计模块规格](./prd/form-design-spec.md)
- [大屏原型 Agent 协作 OpenAPI 规范 V2.0](./prd/agent-data-metrics-api-spec.md)
- [AI 日志监控系统 PRD](./prd/AI-log-monitoring-prd.md)
- [招标网站 / 爬虫 / 全网检索规格](./prd/opportunity-bidding-sites-spec.md)
- [项目机会 / 待推送招标规格](./prd/opportunity-tender-staging-spec.md)
- [风险评分扩展（AI未匹配+自定义风险）](./prd/risk-score-extended-sources-prd.md)
- [评估模板与一键填充](./prd/template-refactor-prd.md)
- [AI 风险评估后端](./prd/ai-risk-assessment-backend-step1.md)
- [一键工作量评估](./prd/workload-one-click-evaluation-prd.md)
- [提示词模板分类重构测试计划](./test/prompt-template-module-tag-refactor.md)
- [小程序会员支付功能规格](./prd/miniapp-membership-payment-spec.md)
- [招标快报小程序 PRD](./prd/miniapp-bidding-prd.md)
...
3. **组织复用与知识沉淀**
   ...
   - Excel 导出一次性通过率 100%，任何手工修订需登记并在 48 小时内修复根因。
...
| 导出能力 | `/api/projects/:id/export/excel` 与实时计算结果一致 | ✅ 已实现 |
...
| IoT 点位对接评估 V1.8 | 标准评估的工作量估算步骤新增 `IoT点位对接` Tab，按 8 项工作包模型独立估算 IoT 对接成本 | ✅ 已实现 |
...
| 表单设计模块 | `/form-design` 可视化编辑表单字段，支持行内编辑、数据校验、Agent 辅助、Excel 导出 | ✅ 已实现 |
| 大屏原型 Agent 协作接口 (V2.0) | 提供 getAgentContext、generateAgentLayout、saveAgentFeedback、convertToPpaTemplate、exportToJson 等 5 个核心双向闭环 API | ✅ 已实现 |
...
- `POST /api/calculate`：输入风险分、角色工作量、IoT 点位对接评估、差旅/维护参数，返回成本拆解、工作量天数和 Rating Factor；IoT 点位对接成本必须作为独立成本项返回，不并入系统对接成本。  
...
- `/api/projects/:id/export/excel`：导出报告接口，需保证输出数据与最新一次 `calculate` 结果一致；`version=business` 基于商务报价快照导出；内部版 Excel 需包含 IoT 点位对接概要与工作包明细。  
...
- **大屏原型 Agent 接口 (V2.0)**：
  - `GET /api/data-metrics/projects/:id/agent-context`：获取极简大纲 (JSON or Markdown)
  - `GET /api/data-metrics/projects/:id/agent-layout`：获取推荐的 Canvas 12栅格自适应布局 DSL
  - `POST /api/data-metrics/projects/:id/agent-feedback`：回写外部 Agent 备注与 3D 配置同步
  - `POST /api/data-metrics/projects/:id/convert-to-ppa-template`：大屏指标一键生成 PPA 项目成本估算模板
  - `GET /api/data-metrics/projects/:id/export/json`：免分页无状态导出指标完整平铺 JSON 列表
...
2. `Assessment/New`：分步向导 + 实时计算；关键字段提供提示信息（例：角色单价输入单位提示）。  
   - 工作量估算步骤包含 `新功能开发`、`系统对接工作量`、`IoT点位对接` 三类 Tab；IoT Tab 文案前不显示图标。
   - IoT Tab 默认展示 8 项工作包，手动规模参数变化时实时刷新建议人天；调整后人天作为最终计价输入，确认后才参与总览。
...
- **FR-2.8 (P0)** `/assessment/new` 的工作量估算步骤必须提供 `IoT点位对接` Tab，按“已有 IoT 平台/网关能力、不包含 IoT 平台建设”的前提进行手动评估；V1.8 不要求上传点表和自动解析。
- **FR-2.9 (P0)** IoT 点位对接评估必须支持 8 项工作包：点表整理、协议/驱动配置、点位映射、数据清洗与单位换算、告警规则配置、批量导入、现场联调、验收核对。每项需展示估算依据、系统建议人天、调整后人天、角色和调整说明。
- **FR-2.10 (P0)** 已确认的 IoT 点位对接成本必须独立计算并在总览展示 `IoT点位对接成本` 与 `IoT点位工作量`；系统对接成本不得包含已确认的 IoT 点位对接成本。总成本 = 软件研发 + 系统对接 + IoT 点位对接 + 差旅 + 运维 + 风险成本。未确认的 IoT 工作包只作为草稿展示，不参与总览。
- **FR-2.11 (P0)** `assessment_details_json` 必须保存 `iot_point_integration`，包含 assumptions、scale_params、generated_items、estimated_point_count、estimated_by_device_count、applied_at；历史项目若只有旧版 `integration_workload` 中的 IoT 行且没有已确认的独立 IoT 数据，必须继续按旧口径计价，避免历史成本丢失。
...
### FR-5 AI 模型配置
详细规格见 [model-config-spec.md](./prd/model-config-spec.md)  
...
- **FR-5.4 (P1)** 提示词模板分类从旧的 `category` 枚举（8个固定值）重构为 `module_tag` + `prompt_module_tags` 字典表模型：模板通过 `module_tag` 字段关联模块标签字典，支持 6 个预置推荐标签（`assessment`、`web3d`、`tender`、`bidding_search`、`report`、`general`），用户也可创建自定义标签。推荐标签不可删除，自定义标签可自由管理。
- **FR-5.5 (P1)** 所有 Web3D Step4 AI 调用必须同步写入数据库日志与文件日志，日志 step 为 `web3d_step4`，并记录图片元数据、最终 prompt、解析结果及未映射项。
- **FR-5.6 (P1)** `GET /api/config/prompt-module-tags` 返回模块标签字典（含 value/label/description/sort_order），支持前缀模糊匹配（如 `module_tag=assessment*`）。

### FR-6 导出能力
用户可在项目详情页或历史列表触发 Excel 导出，后端执行统一导出管线（数据准备 → 模板渲染 → 产出文件），单次导出 ≤ 5 秒；如超时需返回任务 ID 支持重试。
- **FR-6.2 (P0)** 导出内容包含封面（指标概览）、详尽成本拆解、配置摘要，且需标注”计算时间 + Rating Factor”；默认为最新快照，也可勾选历史快照。
...
- **FR-6.8 (P0)** 内部版 Excel 导出必须包含 IoT 点位对接概要 Sheet 与 IoT 工作包明细 Sheet；对外版导出必须将 IoT 点位对接作为独立模块参与报价分摊，且不得与已存在的 `role_costs` IoT 模块重复计入。

### FR-7 系统基础
（API 错误处理由 `middleware/errorHandler.js` 统一输出） API 错误信息统一由 `middleware/errorHandler.js` 输出 JSON，包含错误码与定位信息（保留未来扩展钩子）。

### FR-8 附件管理
详细规格见 [attachment-push-spec.md](./prd/attachment-push-spec.md) 项目详情页新增"附件管理"Tab，支持上传项目相关文档（需求说明书、合同、技术方案等）。
...
### FR-9 项目推送至小程序
详细规格见 [attachment-push-spec.md](./prd/attachment-push-spec.md)

### FR-10 微信小程序内部渠道
详细规格见 [miniapp-internal-channel-spec.md](./prd/miniapp-internal-channel-spec.md) 历史项目列表操作列增加"推送"按钮，点击弹出推送确认 Modal。
...
### FR-11 招标数据采集（爬虫）
详细规格见 [opportunity-bidding-sites-spec.md](./prd/opportunity-bidding-sites-spec.md)

### FR-12 招标全网检索
详细规格见 [opportunity-bidding-sites-spec.md](./prd/opportunity-bidding-sites-spec.md)

### FR-13 小程序会员体系
详细规格见 [miniapp-membership-payment-spec.md](./prd/miniapp-membership-payment-spec.md) Python 爬虫系统定时从招标网站抓取公开招标信息，支持平台：CNOOC（中国海洋石油集团）、CNPC（中国石油天然气集团）、PipeChina（国家管网集团）。
- **FR-11.2 (P2)** 爬虫脚本存放于 `spider/` 目录，文件名按平台 ID 命名（如 `site_119.py`），入口脚本 `spider_service.py` 负责调度。
- **FR-11.3 (P2)** 采集数据落地至 SQLite 表（`tenders`），包含标题、单位、预算、地区、截止时间、原文链接、原始 HTML 等字段。
- **FR-11.4 (P2)** 定时任务默认每日执行一次，支持手动触发；异常时记录日志并重试。

### FR-12 招标全网检索
- **FR-12.1 (P1)** 项目机会模块新增"全网检索"能力，基于 Tavily AI 搜索 API 实时抓取互联网招标信息。
- **FR-12.2 (P1)** 前端 `TenderWebSearchModal` 提供搜索入口，用户输入关键词后调用 `POST /api/ai/tender-web-search`，返回结构化搜索结果。
- **FR-12.3 (P1)** 搜索结果与本地招标信息自动匹配，匹配成功的条目可一键入库，关联已有招标记录。

### FR-13 小程序会员体系
- **FR-13.1 (P0)** 小程序会员三态：`inactive`（无有效会员）、`active`（会员有效）、`expired`（已过期），详情页根据状态执行内容拦截。
- **FR-13.2 (P0)** 会员订单状态：`created` → `pay_pending` → `paid` → `granted`；包含人工补开（`manual_fixed`）和取消（`cancelled`）场景。
- **FR-13.3 (P0)** 有效会员可查看完整招标详情、人工评估结果、成果物（Excel）下载和 Web 服务导流入口。
- **FR-13.4 (P1)** 会员经营分析页（`pages/membership-insights`）提供会员活跃度、到期趋势等最小经营指标。

### FR-14 大屏原型 Agent 协作与双向同步 (V2.0)

#### 14.1 功能规划与业务规格
* **FR-14.1.1 (P0) API-Key 安全访问校验**：为防止越权和接口滥用，所有对外 Agent 接口必须经过 `agentAuthMiddleware` 安全拦截。请求 Headers 中必须包含 `X-Agent-API-Key` 认证。
* **FR-14.1.2 (P0) 大屏幕后上下文提取**：提供极简的树状指标清单上下文。支持一键输出最紧凑、无废话、Markdown 格式的分级树大纲（Token 压缩率极佳，天然适合大语言模型读取与分析）。
* **FR-14.1.3 (P0) Canvas 画布布局自适应推荐**：根据大屏图元指标的 `display_type`（表格、饼图、柱图等），自动输出 12 栅格 Canvas 占位 `grid` 数据，提供前端图元组装和布局推荐的 DSL 底座。
* **FR-14.1.4 (P0) 坐标回写与 3D 参数闭环**：支持第三方协作 Agent 修改完大屏后，一键回写布局坐标并持久化于 `data_metrics` 表备注，并在项目配置中同步 Web3D 的相机视角、fov 等参数，且保持物理层字段完美兼容、绝不报错。
* **FR-14.1.5 (P0) 成本一键转化打通**：支持大屏配置一键转化为 PPA 项目成本估算草稿（`is_template = 1`）。通过大屏指标展示方式的复杂度，智能换算所需前后端研发工时（以 1800~2000 元/人天估价），无缝同步至研发估算流。

---

#### 14.2 对外暴露 OpenAPI 接口设计文档

##### 统一安全头部验证 (Security Header)
```http
X-Agent-API-Key: ppa_agent_secret_token_2026
```

---

##### 1. 获取极简紧凑的大纲上下文 (getAgentContext)
* **请求路径**: `GET /api/data-metrics/projects/:id/agent-context`
* **请求参数 (Query)**:
  * `format` (string, 可选): `markdown` 或 `json`。默认为 `markdown`。
* **返回格式示例 (`format=markdown` / 推荐给大模型)**:
  ```http
  HTTP/1.1 200 OK
...
##### 2. 获取推荐的大屏 12 栅格 Canvas 坐标 DSL (generateAgentLayout)
* **请求路径**: `GET /api/data-metrics/projects/:id/agent-layout`
* **返回格式示例**:
  ```json
  {
    "success": true,
...
##### 3. 外部 Agent 回写排版布局与 3D 参数同步 (saveAgentFeedback)
* **请求路径**: `POST /api/data-metrics/projects/:id/agent-feedback`
* **请求参数 (Body)**:
  ```json
  {
    "layout": [
...
##### 4. 一键转化为 PPA 项目成本估算模板 (convertToPpaTemplate)
* **请求路径**: `POST /api/data-metrics/projects/:id/convert-to-ppa-template`
* **返回格式示例**:
  ```json
  {
    "success": true,
...
##### 5. 免分页指标数据完整平铺 JSON 导出 (exportToJson)
* **请求路径**: `GET /api/data-metrics/projects/:id/export/json`
* **返回格式示例**:
  ```json
  {
    "success": true,
