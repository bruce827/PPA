...
def extract_annex_urls(annex_list: Any) -> str:
    ...
    for item in annex_list:
        ...
        for key in keys:
            ...
            # 只保留看起来像 URL 的值，避免出现纯ID/文件名等无效内容
            if spider_utils._is_likely_url(val_str) or val_str.startswith("/"):
                urls.append(urljoin(BASE_URL, val_str))
    ...
    if isinstance(data, dict):
        for key, val in data.items():
            if isinstance(val, str) and val:
                v = val.strip()
                # 只保留看起来像 URL 的字段
                if spider_utils._is_likely_url(v) or v.startswith("/"):
...
def main():
    ...
    folder = f"pipechina_tenders_{timestamp}"
    json_file, csv_file = spider_utils.save_output_with_attachments(
        records,
        folder,
        attachments_field="annex_urls",
        record_id_field="id",
