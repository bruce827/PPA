...
function buildPayload(prompt, images, maxTokens) {
...
function requestOnce({
  ...
  let url;
  try {
    // 与文本 Gemini Provider 保持一致，使用官方当前更兼容的 v1beta generateContent 路径。
  ...
  const payload = buildPayload(prompt, images, maxTokens);
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        logger.info('Gemini Vision 调用成功', {
          requestHash,
    ...
    req.on('error', (error) => {
      const durationMs = Date.now() - startedAt;
      logger.error('Gemini Vision 网络错误', {
        requestHash,
        message: error.message,
      });
      const err = new HttpError(500, `Gemini Vision 网络错误: ${error.message}`, 'GeminiVisionNetworkError');
    ...
    req.setTimeout(effectiveTimeout, () => {
      req.destroy();
      const durationMs = Date.now() - startedAt;
      logger.error('Gemini Vision 调用超时', { requestHash, durationMs });
      const err = timeoutError(`Gemini Vision 超时 (${effectiveTimeout}ms)`);
      err.durationMs = durationMs;
      reject(err);
    });
...
async function createVisionCompletion(params) {
  ...
  if (!model) {
    throw internalError('Gemini Vision 模型未配置');
  }
  ...
  while (attempt < MAX_RETRIES) {
    ...
    try {
      ...
      logger.warn('Gemini Vision 调用失败，准备重试', {
        attempt,
        delayMs,
  ...
  throw lastError || internalError('Gemini Vision 调用失败');
}
