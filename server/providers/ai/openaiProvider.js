...
const DEFAULT_BASE_URL = process.env.AI_PROVIDER_BASE_URL || 'https://api.openai.com';
...
function requestOnce({ prompt, model, requestHash, api_host, api_key, timeoutMs }) {
  const key = api_key || DEFAULT_API_KEY;
  if (!key) {
    return Promise.reject(internalError('AI_PROVIDER_API_KEY 未配置'));
  }

  const base = api_host || DEFAULT_BASE_URL;
  let url;
  try {
    url = new URL('/v1/chat/completions', base);
  } catch (error) {
    return Promise.reject(internalError('AI_PROVIDER_BASE_URL 配置不正确'));
  }
...
async function createRiskAssessment(params) {
  const model = params.model || process.env.AI_PROVIDER_MODEL || 'gpt-4-turbo';
