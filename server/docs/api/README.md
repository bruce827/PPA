...
api/
├── README.md                 # 本文档
├── API.md                    # Markdown格式的API文档（自动生成）
├── swagger.json              # Swagger/OpenAPI规范文件（自动生成）
├── PPA-API.postman_collection.json  # Postman集合（自动生成）
└── examples/                 # API使用示例
    ├── requests/             # 请求示例
    └── responses/            # 响应示例
```
...
# 仅生成Markdown文档
node scripts/generate-api-docs.js --format=markdown

# 仅生成Swagger文档
node scripts/generate-api-docs.js --format=swagger

# 仅生成Postman Collection
node scripts/generate-api-docs.js --format=postman

# 指定输出目录
node scripts/generate-api-docs.js --format=all --output=./my-docs
```
...
2. 点击 "Import" 按钮
3. 选择 "File" 标签
4. 选择 `PPA-API.postman_collection.json` 文件
5. 点击 "Import" 完成导入
...
| 文档 | 描述 | 用途 |
|------|------|------|
| `API.md` | Markdown格式的API文档 | 快速查阅API列表 |
| `swagger.json` | OpenAPI 3.0规范 | 机器可读的API定义 |
| `PPA-API.postman_collection.json` | Postman集合 | API测试和调试 |
...
1. 导入Postman Collection: `PPA-API.postman_collection.json`
2. 设置环境变量:
...
2. **生成文档**: 运行 `node scripts/generate-api-docs.js`
3. **验证文档**: 检查生成的文档是否正确
4. **提交变更**: 将文档变更提交到版本控制
...
A: 执行以下步骤：
1. 重新生成文档: `node scripts/generate-api-docs.js`
2. 重启服务器
3. 清除浏览器缓存
4. 检查JSDoc注释是否正确
...
2. **Postman**: 导入Postman Collection
3. **Markdown**: 生成Markdown文档
4. **PDF**: 使用第三方工具转换
